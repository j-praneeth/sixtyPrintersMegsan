...
#### 1b. Using the Megascan LIMS instead of lims/

If the cloud LIMS is the **megascan-core-hub** app, skip `lims/supabase/schema.sql`
and `lims/web` entirely — the Megascan repo ships an adapter migration
(`supabase/migrations/20260724100000_print_hub_integration.sql`) that gives its
Supabase project the same surfaces the hub expects (see ARCHITECTURE.md §11).

1. Apply the Megascan migrations to its Supabase project: either
   `supabase db push` (Supabase CLI, linked to the project) from the Megascan
   repo, or paste the print-hub migration into the SQL editor if the rest of
   the schema is already deployed (it only creates new objects — idempotent).
2. Project Settings → API: note the **Project URL** and **service_role** key —
   these go into the hub's Settings tab exactly as in step 2.3 below. The
   `lims-docs` bucket and `documents` table are created by the migration; the
   hub's default bucket name already matches.
3. **Vocabulary rule:** the hub validates jobs by exact string match between a
   registration's device type and the printing device's type. In Megascan the
   registration's device type is its **Technique Name** (Operations → Samples →
   Registration step), and the allowed values are the rows of the new
   `device_types` table (seeded from equipment categories + existing technique
   names). Keep technique names and enrolled device types identical (e.g.
   `HPLC-UV`), or jobs are held with `device_type_mismatch` until assigned.
4. Printed PDFs appear in Megascan under **Operations → Printed Documents**
   (users need `operations` read permission).
...
A job is held (hub dashboard → **Held**) when the registration dialog timed
out unanswered or the registration/test didn't validate (pressing **Cancel**
instead discards the job on the client — it never reaches the hub)
