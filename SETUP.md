...
| Need | Notes |
|---|---|
| **Windows 7 SP1 / 10 / 11 x64** | the printer/port/driver are Windows-only. Win 10/11 need nothing extra; **Windows 7 has extra prerequisites — see below** |
| **Administrator rights** | `install.bat` self-elevates (UAC) |
| **[uv](https://docs.astral.sh/uv/)** | provides the private Python on Win 10/11 (auto-installed); **not used on Windows 7** (see below) |
...
`setup.ps1` detects Windows 7 and switches automatically: printers/drivers are
managed via WMI + `printui.dll` (the PrintManagement cmdlets don't exist there),
and instead of uv + CPython 3.12 (both Win10+-only) it provisions the
**Python 3.8.10 embeddable package** (the last CPython that runs on Win 7 —
`upload.py` is stdlib-only and 3.8-compatible). Ghostscript is pinned to 9.56.1
and qpdf to 10.6.3 (the last builds verified on Win 7). Before running
`install.bat` on a Windows 7 machine you must install, in order:

1. **.NET Framework 4.8** (4.7.2 is the hard minimum — the super-admin password
   hash uses the `Rfc2898DeriveBytes` SHA-256 constructor added in 4.7.2, so
   4.6 fails on the first install)
2. **WMF 5.1** (Windows PowerShell 5.1) — <https://www.microsoft.com/download/details.aspx?id=54616>
3. TLS 1.2 must work for downloads (it does once WMF 5.1/.NET 4.6+ are on); for a
   fully **offline** Win 7 install, pre-drop into `vendor\`:
   `python-3.8.10-embed-amd64.zip`, `gs9561w64.exe`, `qpdf-10.6.3-bin-mingw64.zip`,
   and `mfilemon-setup.exe`.

> Windows 7 has received no security updates since 2020. It is supported here
> for legacy instrument PCs, but keep those machines LAN-only.

### Windows 11 note — Protected Print mode

If **Windows protected print mode** (WPP) is enabled (Settings → Bluetooth &
devices → Printers & scanners, or GPO), Windows blocks third-party port
monitors and this toolkit cannot work. `setup.ps1` detects it and refuses to
install with instructions; turn WPP off, then re-run.
...
- **Automatic prompt (default).** Every time you print, a **"Registration number"**
  ...
  **Cancel** to discard the print job entirely (nothing is uploaded); letting the
  dialog time out sends the job without a number (hub printers are held). Toggle
  off with `"prompt_registration": false` in `config.json`.
